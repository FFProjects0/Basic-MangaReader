...
class MangaReader(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.image_files = sorted([f for f in os.listdir(IMAGE_FOLDER) if f.lower().endswith(('.png', '.jpg', '.jpeg'))], key=natural_key)
    ...
    def on_image_loaded(self, tiles, chapter_name):
        ...
        if AUTO_NEXT == False:
            ...
            next_button.setFixedHeight(100)  # Make it huge
...
class ChapterSelectionDialog(QtWidgets.QDialog):
    def __init__(self, image_files, parent=None):
        ...
        # First, collect entries from Chapters.txt.
        # We'll distinguish between season headers and episode entries.
        entries = []
        for line in lines:
            stripped = line.strip()
            if not stripped:
